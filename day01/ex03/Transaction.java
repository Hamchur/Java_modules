    ...
    public void doTransaction() {
        if (transferAmount >= 0) {
            if (sender.getBalance() < transferAmount) {
                System.err.print("\nInvalid transaction amount: " + transferAmount);
                System.exit(-1);
            }
            sender.setTransferType("CREDIT");
            recipient.setTransferType("DEBIT");
        } else {
            if (Math.abs(recipient.getBalance()) < Math.abs(transferAmount) || recipient.getBalance() < Math.abs(transferAmount)) {
                System.err.print("\nInvalid transaction amount: " + transferAmount);
                System.exit(-1);
            }
            sender.setTransferType("DEBIT");
            recipient.setTransferType("CREDIT");
        }
